  ...
  return (
    <div className="space-y-3">
      <Field label={`${providerLabel(provider)} server`} required error={error} hint="Discovery follows selected WAN">
        <div className="flex gap-2"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="City, sponsor, host, or server ID" aria-label={`${providerLabel(provider)} server search`} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); discover() } }} /></div><Button variant="outline" onClick={discover} disabled={!interfaceName || !sourceIp || serversQuery.isFetching}>{serversQuery.isFetching ? <Spinner /> : <RadioTower className="h-4 w-4" />}Discover</Button></div>
      </Field>
