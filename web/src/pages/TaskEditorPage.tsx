...
export default function TaskEditorPage() {
  ...
  const onSubmit = (data: TaskFormValues) => {
    ...
    if (data.enabled && (preflight?.signature !== signature || !preflight.validation.success)) {
    ...
    <Card>
      ...
      <CardContent className="space-y-5">
        ...
        <div role={preflight?.success ? 'status' : 'alert'} className={cn('flex gap-3 rounded-lg border p-3 text-xs leading-5', preflight?.success ? 'border-emerald-500/25 bg-emerald-500/[.06] text-emerald-700 dark:text-emerald-300' : preflight ? 'border-rose-500/25 bg-rose-500/[.06] text-rose-700 dark:text-rose-300' : 'border-amber-500/25 bg-amber-500/[.06] text-amber-700 dark:text-amber-300')}>
          ...
          <div><p className="font-semibold">{preflight?.success ? 'Current configuration passed preflight' : preflight ? 'Current configuration failed preflight' : configurationChanged ? 'Configuration changed since preflight' : 'Current configuration has not been validated'}</p><p className="mt-0.5">{preflight?.message || (values.enabled ? 'A successful preflight is required before this enabled task can be saved.' : 'Disabled tasks may be saved without preflight, but validation is recommended before enabling.')}</p></div>
        </div>
