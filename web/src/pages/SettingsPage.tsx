...
export default function SettingsPage() {
  ...
  const configurationImportMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      toast({ tone: 'success', title: 'Configuration imported', description: `${result.taskCount} tasks and ${result.routeProfileCount} route profiles restored. Results and EULA acceptance were preserved.` })
    },
  ...
  return (
    <form onSubmit={form.handleSubmit((values) => saveMutation.mutate(values))}>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <Card id="ookla-eula" className="xl:col-span-2"><CardHeader><CardTitle className="flex items-center gap-2"><Scale className="h-4 w-4 text-primary" />Ookla provider licensing</CardTitle><p className="text-xs text-muted-foreground">Explicit operator acknowledgement, independent from CLI installation</p></CardHeader><CardContent><OoklaEulaAcceptance /></CardContent></Card>
        <Card className="xl:col-span-2">
          ...
          <CardContent className="grid gap-3 md:grid-cols-2">
            <div className="flex items-start justify-between gap-4 rounded-lg border border-border bg-muted/30 p-3">
              <div><p className="text-xs font-semibold">Export current configuration</p><p className="mt-1 text-[11px] leading-5 text-muted-foreground">Downloads a versioned JSON file. Results and EULA acceptance are excluded.</p></div>
      ...
      <ConfirmDialog
        ...
        description={pendingConfiguration ? <div className="space-y-2"><p><span className="font-medium text-foreground">{pendingConfiguration.filename}</span> contains {pendingConfiguration.document.tasks.length} tasks and {pendingConfiguration.document.routeProfiles.length} route profiles.</p><p>Saved settings, tasks, and route profiles will be replaced. Measurement results and Ookla EULA acceptance remain unchanged. Import is blocked while a test is active.</p>{isDirty ? <p className="text-amber-500">Unsaved edits on this page will be discarded.</p> : null}</div> : ''}
