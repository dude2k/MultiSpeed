        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className="pointer-events-auto flex items-start gap-3 rounded-xl border border-border bg-card p-4 text-card-foreground shadow-2xl">
              ...
              <div className="min-w-0 flex-1"><p className="text-sm font-semibold">{t(item.title)}</p>{item.description ? <p className="mt-1 text-xs leading-5 text-muted-foreground">{t(item.description)}</p> : null}</div>
              <button type="button" onClick={() => remove(item.id)} className="rounded p-1 text-muted-foreground hover:bg-accent" aria-label={t('Dismiss notification')}><X className="h-3.5 w-3.5" /></button>
            </div>
          )
        })}
      </div>
