...
function Brand() {
  return (
    <NavLink to="/" className="group flex items-center gap-3 rounded-lg focus-visible:ring-2 focus-visible:ring-ring" aria-label="MultiSpeed dashboard">
      ...
      <span>
        ...
        <span className="block text-[10px] font-semibold uppercase tracking-[.15em] text-muted-foreground">Path telemetry</span>
      </span>
    </NavLink>
  )
}
...
function ConnectionBadge({ state }: { state: StreamState }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg border border-border bg-background px-2.5 py-2 text-[11px] font-medium text-muted-foreground" title="Server-sent events connection">
      ...
      {connected ? 'Live' : state === 'unavailable' ? 'Offline' : 'Reconnecting'}
    </div>
  )
}
    ...
    <div className="min-h-screen bg-background">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-border bg-card/95 px-4 py-5 lg:flex lg:flex-col">
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4">
          ...
          <p className="px-1 text-[10px] leading-4 text-muted-foreground">No authentication. Keep this console on a trusted network.</p>
        </div>
      </aside>
        ...
        <header className="sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur-xl">
          <div className="flex h-16 items-center gap-3 px-4 sm:px-6 lg:px-8">
            <DialogPrimitive.Root open={mobileOpen} onOpenChange={setMobileOpen}>
              <DialogPrimitive.Trigger asChild><Button size="icon" variant="ghost" className="lg:hidden" aria-label="Open navigation"><Menu className="h-5 w-5" /></Button></DialogPrimitive.Trigger>
              <DialogPrimitive.Portal>
                ...
                <DialogPrimitive.Content className="fixed inset-y-0 left-0 z-50 w-[min(88vw,19rem)] border-r border-border bg-card p-5 shadow-2xl outline-none">
                  ...
                  <DialogPrimitive.Description className="sr-only">Navigate between MultiSpeed screens</DialogPrimitive.Description>
                  <div className="flex items-start justify-between gap-3"><Brand /><DialogPrimitive.Close asChild><Button size="icon" variant="ghost" aria-label="Close navigation"><X className="h-4 w-4" /></Button></DialogPrimitive.Close></div>
                  <div className="mt-7"><Navigation onNavigate={() => setMobileOpen(false)} /></div>
                  <div className="absolute inset-x-5 bottom-5 space-y-3"><ConnectionBadge state={state} /><ThemeControl mode={theme} setMode={setTheme} /></div>
                </DialogPrimitive.Content>
              </DialogPrimitive.Portal>
            </DialogPrimitive.Root>
            <div className="min-w-0 flex-1">
              <p className="truncate text-[10px] font-bold uppercase tracking-[.13em] text-primary">{meta.eyebrow}</p>
